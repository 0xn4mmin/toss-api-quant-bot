...
## 7.5 ★ 현행 체제: 연구용 순방향 검증 (2026-07-08 결정)

백테스트 5회 판정 결과, 주식·자산군 롱온리로는 이 OOS 구간에서 MDD 예산
(INV-04)을 충족하지 못했다 (최종: dual-momentum.v3 — 샤프 0.75 PASS,
MDD 22.2%/p95 21.9% FAIL). 반복 중단 합의 이행 — 예산을 결과에 재단하지
않고, **오염되지 않는 유일한 OOS인 미래 데이터**로 전환한다:

- [ ] 연구용 페이퍼 상시 가동 (승격 경로 없음 — LC-G2/INV-08이 구조적 차단):
  ```
  $ uv run quantbot run --strategy strategies/dual-momentum.v3.yaml
  ```
  파라미터는 판정 아티팩트에서 자동 로드(13주/top2), 매주 실행창에서 사이클,
  월간 재선정 + 주간 음전 방어, INV-10 주문 분할, 아침 보고서 텔레그램 발신.
  재시작 안전(registry 재생). **⚠ run 계열은 자동 테스트가 없는 조립 코드 —
  첫 사이클은 감독하에**
- [ ] 3~6개월 순방향 실적 축적 후 INV-04 예산 재논의 (그 사이 KR flows 표본이
  이 봇의 고유 엣지를 준비한다 — 7번 cron이 그래서 급하다)
- 참고: 시뮬·페이퍼 모두 현금 이자 0 모델(보수 방향) — 실제는 +1~2%p

## 8. 페이퍼 1개월 (LC-G3) — 승격의 전제 (게이트 통과 전략이 생기면)[text](https://portal.hanyang.ac.kr/port.do)
